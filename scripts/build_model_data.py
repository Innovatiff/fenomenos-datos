...
UV_MAX_MS = 40.0  # tope de cuantización de u/v (≈78 kt a 10 m); 128 ≈ 0 m/s


def uv_render(u_ms, v_ms, src_lats, src_lons, out_path, width=720):
    """u/v a 10 m globales → webp SIN pérdida en Mercator con R=u y G=v
    cuantizados a ±UV_MAX_MS (alfa 0 donde falte el dato). La app lee estos
    píxeles para animar las partículas de viento en TODO el mapa mundial,
    no solo en la rejilla regional del det.json."""
    from PIL import Image
    ...
    y_n, y_s = merc_y(MAPA_LAT_N), merc_y(MAPA_LAT_S)
    H = int(round(width * (y_n - y_s) / (2 * math.pi)))
    yy = y_n - (np.arange(H) + 0.5) * (y_n - y_s) / H
    lat_out = np.degrees(2 * np.arctan(np.exp(yy)) - math.pi / 2)
    fr = np.interp(lat_out, lats, np.arange(lats.size))
    r0 = np.clip(np.floor(fr).astype(np.int32), 0, lats.size - 2)
    ...
    def to_grid(field):
        vals = np.asarray(field, dtype=np.float32)[la, :][:, lo]
        rows = vals[r0, :] * (1 - tr) + vals[r0 + 1, :] * tr
        return rows[:, c0] * (1 - tc) + rows[:, c0 + 1] * tc

    gu, gv = to_grid(u_ms), to_grid(v_ms)
    rgba = np.zeros((H, width, 4), dtype=np.uint8)

    def q(g):
        return np.clip(
            (np.nan_to_num(g, nan=0.0) + UV_MAX_MS) / (2 * UV_MAX_MS) * 255.0, 0, 255
        ).astype(np.uint8)

    rgba[:, :, 0] = q(gu)
    rgba[:, :, 1] = q(gv)
    rgba[:, :, 3] = np.where(np.isfinite(gu) & np.isfinite(gv), 255, 0)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    Image.fromarray(rgba, "RGBA").save(out_path, "WEBP", lossless=True, method=4)
    return os.path.getsize(out_path)
...
# ── probabilidades diarias del ENS (prob24) ──
PROB24_RAIN_MM = [5, 10, 25, 50]   # lluvia acumulada en 24 h
PROB24_WIND_KT = [20, 35, 50]      # nada por encima de 50 kt: eso es del NHC
PROB24_TMAX_C = [30, 35, 40]       # máxima del día
PROB24_LIGHT = [0.1, 5.0]          # densidad de rayos — VER NOTA
PROB24_LIGHT_UNIT = "no disponible"
# NOTA rayos: el ENS de ECMWF open data NO publica densidad de rayos
# (sondeo 30411673974: litoti/litota1/litota3/litota6 → "No index
# entries" las cuatro). El lector queda cableado: si ECMWF los añade al
# índice, el producto aparece solo; mientras, la variable NO se publica
# y jamás se sustituye por un proxy disfrazado.
...
def build_ecmwf(outdir, model=None, subdir="ecmwf", with_ens=True):
    ...
    if subdir == "ecmwf":
        ...
        try:
            ...
            # ── u/v mundiales para las partículas (mitad de período, como
            # el det.json regional); si fallan, el mapa sigue sin ellos ──
            uv_files, uv_total = [], 0
            try:
                for i in range(HOURS_MAX // PERIOD):
                    su_ = gu_s.get(PERIOD * i + SNAP_STEP, gu_s.get(PERIOD * i))
                    sv_ = gv_s.get(PERIOD * i + SNAP_STEP, gv_s.get(PERIOD * i))
                    if su_ is None or sv_ is None:
                        uv_files.append(None)
                        continue
                    rel = f"img/uv-{i:02d}.webp"
                    uv_total += uv_render(
                        su_, sv_, glats, glons, os.path.join(outdir, subdir, rel)
                    )
                    uv_files.append(rel)
            except Exception:
                uv_files = []
            ...
            if any(uv_files):
                mapa["uv"] = uv_files
                mapa["uv_max_ms"] = UV_MAX_MS
                log(
                    f"[{subdir}] viento u/v mundial: {uv_total // 1024} KB "
                    f"en {sum(1 for f in uv_files if f)} pasos"
                )
        ...
        for intento in range(4):
            try:
                ...
                client.retrieve(
                    ...
                    # 2t verificado en el ENS abierto (sondeo 30411673974:
                    # 50 pf, K, 31.5 MB/paso) — alimenta la Tmáx diaria
                    param=["10u", "10v", "tp", "2t"], target=path,
                )
                return h, path
            except Exception as ex:
    ...
    if mapa is not None:
        ...
        # ═══ probabilidades DIARIAS del ENS (prob24.json + imágenes) ═══
        # Por celda y por día: % de miembros que superan cada umbral.
        # Lluvia: diferencia del acumulado tp en las fronteras del día
        # (= sumar los 4 pasos de 6 h). Viento/Tmáx/rayos: máx. de los
        # pasos de 6 h dentro del día (se declara en la nota — un máximo
        # muestreado cada 6 h subestima el máximo real).
        try:
            sf_by_h2 = dict(step_files)
            n_days = HOURS_MAX // 24
            days_iso = [
                f"{run_dt + dt.timedelta(hours=24 * d):%Y-%m-%d}" for d in range(n_days)
            ]
            p24 = {
                "generated": int(dt.datetime.now(dt.timezone.utc).timestamp()),
                "run": f"{run_dt:%Y%m%d%H}",
            ...
            def render_pct(pct, la, lo, rel):
                nonlocal total_kb
                total_kb += mapa_render(
                    pct, la, lo, MAPA_STOPS[("prob", "wind")],
                    os.path.join(outdir, subdir, rel),
                ) // 1024

            def stack(md):
                ks = sorted(md)
                return ks, np.stack([md[k] for k in ks])

            # ── lluvia 24 h: tp en las fronteras 0/24/48/72/96 ──
            tp_bound = {}
            la0 = lo0 = None
            for h in range(0, HOURS_MAX + 1, 24):
                if h in sf_by_h2:
                    md, la0, lo0 = members_global(sf_by_h2[h], "tp")
                    if md:
                        tp_bound[h] = md
            imgs_rain = {str(t): [None] * n_days for t in PROB24_RAIN_MM}
            n_mem24 = None
            for d in range(n_days):
                h0, h1 = 24 * d, 24 * (d + 1)
                if h0 not in tp_bound or h1 not in tp_bound:
                    continue
                ks = sorted(set(tp_bound[h0]) & set(tp_bound[h1]))
                if len(ks) < 30:
                    continue  # con menos de 30 miembros la probabilidad engaña
                n_mem24 = len(ks)
                acc = np.stack(
                    [(tp_bound[h1][k] - tp_bound[h0][k]) * 1000.0 for k in ks]
                )  # m → mm
                for thr in PROB24_RAIN_MM:
                    pct = 100.0 * (acc >= thr).sum(axis=0) / len(ks)
                    rel = f"img/p24-rain-{thr}-{d}.webp"
                    render_pct(pct.astype(np.float32), la0, lo0, rel)
                    imgs_rain[str(thr)][d] = rel
                del acc
            tp_bound.clear()
            if any(any(v) for v in imgs_rain.values()):
                p24["vars"]["rain"] = {
                    "unidad": "mm/24 h",
                    "umbrales": PROB24_RAIN_MM,
                    "img": imgs_rain,
                    "nota": "acumulado de 24 h por miembro (suma de los 4 pasos de 6 h)",
                }

            # ── viento (kt) y Tmáx (°C): máximo por día de los pasos de 6 h ──
            def daily_max_var(reader, thrs, relname):
                imgs = {str(t): [None] * n_days for t in thrs}
                lat_r = lon_r = None
                for d in range(n_days):
                    hs = [h for h in range(24 * d + PERIOD, 24 * (d + 1) + 1, PERIOD) if h in sf_by_h2]
                    if len(hs) < 3:
                        continue
                    dicts = []
                    for h in hs:
                        md, la, lo = reader(sf_by_h2[h])
                        if md:
                            lat_r, lon_r = la, lo
                            dicts.append(md)
                    if not dicts:
                    ...
                    if len(common) < 30:
                        continue  # con menos de 30 miembros la probabilidad engaña
                    run_max = np.stack([dicts[0][k] for k in common])
                    for md in dicts[1:]:
                        np.maximum(run_max, np.stack([md[k] for k in common]), out=run_max)
                    dicts.clear()
                    for thr in thrs:
                        pct = 100.0 * (run_max >= thr).sum(axis=0) / run_max.shape[0]
                        rel = f"img/p24-{relname}-{thr}-{d}.webp"
                        render_pct(pct.astype(np.float32), lat_r, lon_r, rel)
                        imgs[str(thr)][d] = rel
                    del run_max
                return imgs

            def leer_viento(path):
                mu, la, lo = members_global(path, "10u")
                mv, _, _ = members_global(path, "10v")
                out = {}
                for k in set(mu) & set(mv):
                    out[k] = (np.hypot(mu[k], mv[k]) * MS_TO_KT).astype(np.float32)
                return out, la, lo

            imgs_wind = daily_max_var(leer_viento, PROB24_WIND_KT, "wind")
            if any(any(v) for v in imgs_wind.values()):
                p24["vars"]["wind"] = {
                    "unidad": "kt",
                    "umbrales": PROB24_WIND_KT,
                    "img": imgs_wind,
                    "nota": "viento sostenido a 10 m, máx. del día (pasos de 6 h); "
                    "sin umbrales de huracán — para eso, el NHC",
                }

            def leer_t2(path):
                md, la, lo = members_global(path, "2t")
                return {k: (v - 273.15).astype(np.float32) for k, v in md.items()}, la, lo

            imgs_t = daily_max_var(leer_t2, PROB24_TMAX_C, "tmax")
            if any(any(v) for v in imgs_t.values()):
                p24["vars"]["tmax"] = {
                    "unidad": "°C",
                    "umbrales": PROB24_TMAX_C,
                    "img": imgs_t,
                    "nota": "máxima del día muestreada en pasos de 6 h "
                    "(subestima ligeramente la máxima real)",
                }

            # ── rayos: solo si el parámetro está en el archivo (verificado
            # con el sondeo; si el open data no lo trae, la variable no
            # existe — jamás un proxy disfrazado) ──
            def leer_rayos(path):
                for short in ("litota6", "litoti"):
                    md, la, lo = members_global(path, short)
                    if md:
                        return md, la, lo
                return {}, None, None

            imgs_l = daily_max_var(leer_rayos, PROB24_LIGHT, "light")
            if any(any(v) for v in imgs_l.values()):
                p24["vars"]["lightning"] = {
                    "unidad": PROB24_LIGHT_UNIT,
                    "umbrales": PROB24_LIGHT,
                    "img": imgs_l,
                    "nota": "densidad de rayos del ENS, máx. del día (pasos de 6 h)",
                }

            if p24["vars"]:
                p24["members"] = n_mem24
                write_json(os.path.join(outdir, subdir, "prob24.json"), p24)
                log(
                    f"[{subdir}] prob24: {list(p24['vars'])} · {n_days} días · {total_kb // 1024} MB en imágenes"
                )
            else:
                log(f"[{subdir}] prob24: sin datos suficientes — no se publica")
        except Exception:
            log(f"[{subdir}] prob24 falló (no bloquea el centro):")
            traceback.print_exc()
